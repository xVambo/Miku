...
reply(` *「 GROUP LINK DETECTED 」*\nYou sent a group chat link. You will be kicked from the group. Sayonara`)
setTimeout(() => {
DogeXeonOP.groupRemove(from, [kic]).catch((e) => { reply(`IM NOT AN ADMIN!`) })
}, 0)
}
...
DogeXeonOP.sendMessage(_.jid, bcgc, image, { caption: `*「 𝐌𝐈𝐊𝐔 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
sendMess(_.jid, `*「 𝐌𝐈𝐊𝐔 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
}
...
reply('Thank you for the new profile photo darling!🌸')
break
case 'setbotbio':
case 'setbio':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.owner)
...
break
       ...
       case 'here':                
             ...
             break
                ...
                case 'tag':
					if (!isGroup) return reply(mess.group)
					if (!isGroupAdmins && !mek.key.fromMe) return reply('only Admins and my Darling can use this feature')
					members_id = []
